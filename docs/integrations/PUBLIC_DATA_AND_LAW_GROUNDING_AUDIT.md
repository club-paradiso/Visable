...
## Phase 4 controlled `/api/ask` integration (2026-05-19)

- Added controlled law-grounding integration path in `/api/ask` behind strict gating:
  - `LAW_GROUNDING_MODE` must be `audit` or `enabled`
  - legal-intent detection must match explicit 법령/조문 signals
- Default remains disabled; no behavior change when `LAW_GROUNDING_MODE=disabled`.
- Manual grounding remains higher priority for verified operational guidance (documents/procedure).
- Law grounding is not used to infer required document lists, deadlines, fees, or procedural steps.
- Failure behavior is graceful:
  - law grounding warnings/unavailable states stay in metadata
  - `/api/ask` continues normal flow and does not fail because of law grounding errors
- Mock-tested only in this phase; no live external API dependency in tests.
