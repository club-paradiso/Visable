# Remaining Batch R03 — D-10, E-1, E-2, E-3, E-4, E-5
...
- Mechanical re-scan (procedure arrays, ID arrays, subCodes, documents_*): Type N = 1 (구직활동계획서 vs 구직활동 계획서) — previously closed as AMBIGUOUS_TABLE_CONTEXT in merged PR #327 (different applicant sub-categories, 점수제 적용 vs 면제 특례); NOT re-opened, no new evidence.
...
- `documents_extension` (10 entries): **no `_source_notes` provenance** → governing manual undeterminable → Type C/D = `AMBIGUOUS_MANUAL_MISMATCH` (skip); Type A checked = 0.
- ID-array rendered-label duplicate scan (two doc ids → same ko_name): 0.
- **Confirmed fix applied (Type D, CONFIRMED_EXACT):**
    - `documents_extension[9].name`: `결핵건강진단서` → `결핵진단서`
      Authority: 외국인체류 안내매뉴얼 2026.5 (extension tab). Evidence: stay manual
      L453–477 '외국인 결핵진단서 제출 의무 관련 안내' + '결핵 고위험국가(35개국)'
      (matches entry note '보건소/지정병원, 고위험 35개국 필수'); '결핵건강진단서' = 0 hits
      in both manuals, '결핵진단서' = 14 hits each; distinct doc '결핵검진확인서' verified
      separate (F-2/영주/K-STAR contexts). Table context: clear (prose section).
...
### E-2
- Procedure tabs inspected: extension, reentry, registration, statusChange, workplaceChange
- Mechanical re-scan (procedure arrays, ID arrays, subCodes, documents_*): 0 Type A, 0 Type B, 0 Type E, 0 Type N
- `documents_initial` (6 entries): **no `_source_notes` provenance** → governing manual undeterminable → Type C/D = `AMBIGUOUS_MANUAL_MISMATCH` (skip); Type A checked = 0.
- `documents_extension` (8 entries): **no `_source_notes` provenance** → governing manual undeterminable → Type C/D = `AMBIGUOUS_MANUAL_MISMATCH` (skip); Type A checked = 0.
- ID-array rendered-label duplicate scan (two doc ids → same ko_name): 0.
- Confirmed fixes this run: **0**.
...
**Batch totals:** confirmed fixes = 1; ambiguous/skip entries recorded = 1; validation = JSON valid, no diff produced.
