# Remaining Batch R07 — D-5, D-6, D-4-2K, K-STAR, REGION-S, YOUTH-STAY
...
- `documents_extension` (2 entries): **no `_source_notes` provenance** → governing manual undeterminable → Type C/D = `AMBIGUOUS_MANUAL_MISMATCH` (skip); Type A checked = 0.
- ID-array rendered-label duplicate scan (two doc ids → same ko_name): 0.
- Confirmed fixes this run: **0**.
...
- Procedure tabs inspected: extension, registration
- Mechanical re-scan (procedure arrays, ID arrays, subCodes, documents_*): 0 Type A, 0 Type B, 0 Type E, 0 Type N
- documents_* arrays: none.
- ID-array rendered-label duplicate scan (two doc ids → same ko_name): 0.
- Confirmed fixes this run: **0**.

### YOUTH-STAY
- Procedure tabs inspected: statusChange
- Mechanical re-scan (procedure arrays, ID arrays, subCodes, documents_*): 0 Type A, 0 Type B, 0 Type E, 0 Type N
- documents_* arrays: none.
- ID-array rendered-label duplicate scan (two doc ids → same ko_name): 0.
- Confirmed fixes this run: **0**.
