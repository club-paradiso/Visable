...
function lawConfig() {
  const oc = String(process.env.LAW_API_OC || '').trim();
  ...
  const baseUrl = /^https:\/\/([a-z0-9-]+\.)*law\.go\.kr(?::\d+)?$/i.test(rawBase)
    ...
    : DEFAULT_BASE_URL;
  return {
    credential: oc || legacy,
    credentialSource: oc ? 'LAW_API_OC' : (legacy ? 'LAW_API_KEY' : ''),
    mode,
    baseUrl,
  };
}
...
function publicLawConfig() {
  const cfg = lawConfig();
  return {
    lawApiConfigured: Boolean(cfg.credential),
    lawApiCredentialSource: cfg.credentialSource || null,
...
async function lawDetail(candidate, cfg) {
...
async function groundOfficialLaw(caseData, baseline) {
  ...
  if (!cfg.credential) {
    base.limitations.push('Vercel에 LAW_API_OC 또는 LAW_API_KEY가 없어 정적 검증 스냅샷을 사용합니다.');
    return base;
  }
  ...
  if (rule.ok) {
    ...
    appendix7 = findAppendix7(rule.data);
    if (rule86) {
      base.evidence.push(evidenceItem({
