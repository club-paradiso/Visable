...
function findAppendix7(data) {
  for (const object of allObjects(data)) {
    const number = firstText(object, ['별표번호', '별표번호문자열', 'appendixNumber', 'appendixNo']);
    const kind = firstText(object, ['별표구분', '별표종류', 'appendixType']);
    const title = firstText(object, ['별표제목', '별표제목문자열', 'appendixTitle', 'title']);
    const body = firstText(object, ['별표내용', 'appendixText', '내용', 'text']);
    const normalizedNumber = String(number || '').replace(/\D/g, '');
    const explicitSeven = normalizedNumber === '7';
    const appendixKind = !kind || /별표|1/.test(kind);
    const content = cleanExcerpt([title, body].filter(Boolean).join(' '), 1200);
    if (explicitSeven && appendixKind && /범칙금|양정기준/.test(content)) {
      return cleanExcerpt([`별표 ${number}`, title, body].filter(Boolean).join(' '), 850);
    }
    ...
    const blob = cleanExcerpt(Object.entries(object)
      ...
      .join(' '), 1200);
    if ((/별표\s*(?:제)?\s*7\b/.test(blob) || /별표번호\s*7\b/.test(blob)) && /범칙금|양정기준/.test(blob)) {
