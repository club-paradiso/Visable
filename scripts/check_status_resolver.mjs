...
check('A. unknown subcode never becomes a real status', () => { const r = run('F-9-9 연장'); assert(r.step.kind === 'no-status'); const u = run('F-1-77 연장'); assert(u.interp.unknownCodes.includes('F-1-77') && u.interp.codes[0].code === 'F-1', 'falls back to the parent with a warning'); assert(u.html.includes('확인되지 않는 코드')); });
...
check('evidence: every resolved answer exposes at least one 2026.9 page with the manual title', () => { for (const q of ['F-2-7 연장', 'E-9 호텔 연장', 'D-2 연장', 'G-1-5 연장', 'F-6-1 외국인등록']) { const r = run(q); assert(r.model.evidence.length >= 1 && r.model.evidence.every((e) => e.page && e.file && e.date === '2026-09-18'), q); } });
