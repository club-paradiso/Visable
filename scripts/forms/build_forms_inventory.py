...
Sources (all local, offline):
  * 출입국관리법 시행규칙 (법무부령 제1106호, 2026-01-23) — every 별지 서식 marker in
    ...
    the annex → page map is pinned to that PDF's sha256 in data/forms_rule_1106_annex_pages.json
    so the output is identical with or without PyMuPDF installed);
...
PAGE_MAP = os.path.join(ROOT, 'data/forms_rule_1106_annex_pages.json')  # annex → PDF page, pinned to the rule PDF sha256
...
def compute_rule_pdf_pages():
    """Annex number → first page in the rule PDF, read from the PDF text. Needs PyMuPDF;
...
def rule_pdf_pages():
    """Pinned annex → page map, so the inventory never depends on whether PyMuPDF is
    installed. With PyMuPDF the map is recomputed from the PDF (and --check compares it
    with the committed PAGE_MAP); without it the committed map is used as long as the
    rule PDF's sha256 still matches the one recorded in it.
    Returns (pages, map_document)."""
    pdf_sha = sha256(os.path.join(ROOT, RULE_PDF))
    computed = compute_rule_pdf_pages()
    if computed is not None:
        return computed, {'source_pdf': RULE_PDF, 'sha256': pdf_sha, 'generator': 'scripts/forms/build_forms_inventory.py', 'pages': computed}
    committed = json.load(open(PAGE_MAP, encoding='utf-8')) if os.path.exists(PAGE_MAP) else None
    if committed and committed.get('sha256') == pdf_sha and committed.get('source_pdf') == RULE_PDF:
        return committed['pages'], committed
    sys.exit('[build_forms_inventory] PyMuPDF is not installed and ' + ('the rule PDF no longer matches ' if committed else 'there is no ') +
             os.path.relpath(PAGE_MAP, ROOT) + ' — run `pip install pymupdf` and re-run this script')
...
def main():
    ...
    pages, page_map = rule_pdf_pages()
    page_map_text = json.dumps(page_map, ensure_ascii=False, indent=2) + '\n'
    for e in rule:
    ...
    if a.check:
        ...
        cur_map = open(PAGE_MAP, encoding='utf-8').read() if os.path.exists(PAGE_MAP) else ''
        stale = [name for name, cur, new in (('data/forms_inventory.json', cur_json, json_text), ('coverage report', cur_md, md_text), ('annex page map', cur_map, page_map_text)) if cur != new]
        if stale:
            print('[build_forms_inventory] STALE (' + ', '.join(stale) + ') — run: python3 scripts/forms/build_forms_inventory.py', file=sys.stderr)
            sys.exit(1)
        print('[build_forms_inventory] OK — inventory and coverage report are current')
        return
    open(OUT_JSON, 'w', encoding='utf-8').write(json_text)
    open(PAGE_MAP, 'w', encoding='utf-8').write(page_map_text)
