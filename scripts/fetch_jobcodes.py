#!/usr/bin/env python3
"""Build Paradiso job/industry-code runtime data from KSCO8 and KSIC11 seeds.

Paradiso's HiKorea employment-information helper must show both:
  - 직종: 제8차 한국표준직업분류(KSCO8)
  - 업종: 제11차 한국표준산업분류(KSIC11)

The earlier runtime data used a mixed public-data dump that made the UI diverge
from the actual HiKorea employment-information reporting flow. This script now
builds a conservative major/middle seed for occupation codes and a major-group
seed for industry codes until full table extraction is validated.
"""
...
def build_payload(job_records: list[dict[str, str]], industry_records: list[dict[str, str]]) -> dict:
    ...
    return {
        "source": "통계청 표준분류 기반 취업정보 신고용 직종/업종 seed",
        ...
        "coverage": {
            "note": "Seed data only. Full KSCO8 1,270 detailed-unit and full KSIC11 lower-level extraction require later validation PRs.",
            "occupation_levels": ["major", "middle"],
            "industry_levels": ["major"],
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    job_records = load_seed(KSCO8_SEED)
    industry_records = load_seed(KSIC11_SEED)
    payload = build_payload(job_records, industry_records)
    if "--check" in argv:
        print(json.dumps({"categories": payload["categories"], "coverage": payload["coverage"]}, ensure_ascii=False, indent=2))
        return 0
