...
_DEFAULT_TOTAL_BUDGET_SECONDS = 12.0
...
class GroundingConfig:
    ...
    def law_api_credential(self) -> str:
        ...
        The public API contract is OC-first in every environment. Deployment
        platform heuristics must never silently replace an explicitly supplied
        identifier with a different credential.
        """
...
def _parse_total_budget(raw_value: str | None) -> float:
    if raw_value is None or raw_value.strip() == "":
        return _DEFAULT_TOTAL_BUDGET_SECONDS
    try:
        return min(30.0, max(1.0, float(raw_value)))
    except (TypeError, ValueError):
        return _DEFAULT_TOTAL_BUDGET_SECONDS
...
def load_grounding_config() -> GroundingConfig:
    ...
    return GroundingConfig(
        ...
        total_budget_seconds=_parse_total_budget(
            os.environ.get("LAW_GROUNDING_TOTAL_BUDGET_SECONDS")
        ),
