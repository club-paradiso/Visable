...
| `OPENROUTER_FALLBACK_RESERVE_SECONDS` | optional | Defaults to `12` (constrained to 0–20). Prevents one slow candidate from consuming the entire chain budget before an alternate can run. |
| `ENFORCEMENT_AI_BUDGET_SECONDS` | optional | Defaults to `8` (constrained to 1–12). Total wall-clock budget for the whole enforcement model chain. Enforcement is synchronous for the user, so a long fallback chain is worse than returning the deterministic legal baseline alone. The value is passed **into** the candidate chain, which sizes each attempt from it and reserves half of it for an alternate model — a slow first candidate can no longer consume the budget on its own. |
| `ENFORCEMENT_AI_BUDGET_GRACE_SECONDS` | optional | Defaults to `1.5` (constrained to 0.5–5). Extra time before the outer backstop cancels the enforcement chain. The chain normally returns its own metadata when its budget runs out; the backstop exists only for a chain that fails to return at all, and cancelling it earlier discards the attempted-model list, the upstream statuses and the cooldown marking. |
| `OPENROUTER_MIN_CANDIDATE_ATTEMPT_SECONDS` | optional | Defaults to `3.5` (constrained to 0.5–10). Smallest slice a candidate is worth giving when a caller supplies its own chain budget. That budget is split evenly across the candidates still to try, so more of them means more independent tries rather than one long wait — live enforcement probes that succeeded returned in about three seconds, while the failures hung to their cap. Slicing stops at this floor instead of handing out slivers that would cut off an answer that was coming. Does not affect the default 45s chain. |
| `PARADISO_BUILD_COMMIT` | optional | The commit this process was built from, reported at `/health` as `build.commit`. On Railway the built-in `RAILWAY_GIT_COMMIT_SHA` is picked up automatically (then `SOURCE_COMMIT`, then `GIT_COMMIT`); set this only on a host that provides none of them. Without it a post-deploy check cannot tell a finished deploy from the previous one still answering `ok`, and silently tests the code the deploy replaced. Values must look like a commit sha or they are ignored — `/health` is public and never echoes an arbitrary env value. |
